...
def load_page_debug():
    from ui.pages.debug import show_debug_page
    return show_debug_page
...
def show_sidebar():
    ...
    with st.sidebar:
        st.markdown("### ExcelAI")
        st.markdown("**v0.3** - AI Excel Sistemi")
        st.divider()
        ...
        with col1:
            st.metric("Araç", "6")
        with col2:
            from core.database import count_functionalities
            active_business = st.session_state.get("active_business")
            business_id = active_business["id"] if active_business else None
            func_count = count_functionalities(business_id) if business_id else 0
            st.metric("İş", func_count)
...
def main():
    ...
    try:
        if current_page == "dashboard":
            ...
            show_functions_page = load_page_functions()
            show_functions_page()
            ...
            show_debug_page = load_page_debug()
            show_debug_page()
